...
//	    printf(" %d ",vect[i]);
	...
	fclose(fileptr);
        ...
        for (i = 0; i < nworkers; i++) // enviar porciones de arreglo a los workers, primero mandamos la porcion mas grande
        {
            ...
            MPI_Send(&auxvect, dimworker, MPI_INT, (i + 1), (i + 1), MPI_COMM_WORLD);
        }
        ...
        for (v = 0; v < cantvueltas; v++)
        {
            ...
            for (i = 0; i < cantworkers; i++) // iteramos por cada worker que debe recibir una dupla de filas para combinarlas
            {
                ...
                MPI_Send(&matriz[offset], cantcolumnas, MPI_INT, workerid, workerid, MPI_COMM_WORLD);//se sacaron los isend
                MPI_Send(&matriz[offset + cantcolumnas], cantcolumnas, MPI_INT, workerid, workerid, MPI_COMM_WORLD);
                workerid++;
	...
	fprintf(fileptr, "Cantidad de nodos: %d\n", nproc);
	fprintf(fileptr, "Cantidad de workers: %d\n", nworkers);
...
//	printf("\nPrueba pre quicksort del worker %d\n", myId);
//	for (i = 0; i< 10; i++){
//		printf(" %d ",vectworker[i]);
//	}
//	printf("\n");
        quicksort(vectworker, 0, (dimworker - 1));
//	printf("\nPrueba post quicksort del worker %d\n", myId);
//	for (i = 0; i<10; i++){
//		printf(" %d ",vectworker[i]);
//	}
	    ...
	    int limiteworker = nworkers;
            ...
            MPI_Send(filaresultado, (2 * cantcolumnas), MPI_INT, 0, myId, MPI_COMM_WORLD);// se saco el isend
